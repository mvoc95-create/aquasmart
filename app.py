...
ROLE_PERMISSIONS = {
    'admin': {
        ...
        'transfers_manage', 'feed_manage', 'sales_manage', 'users_manage', 'documents_manage'
    },
    'gerente': {
        'dashboard', 'units_view', 'lots_manage', 'water_manage', 'management_manage',
        'transfers_manage', 'feed_manage', 'sales_manage', 'documents_manage'
    },
    'operador': {
        'dashboard', 'units_view', 'water_manage', 'management_manage',
        'transfers_manage', 'feed_manage', 'documents_manage'
    },
    'consulta': {'dashboard', 'units_view', 'documents_manage'},
}
...
def management_page():
    if request.method == 'POST':
        ...
        return redirect(url_for('management_page', unit_id=unit_id if unit_id else ''))
    units = Unit.query.filter_by(active=True).order_by(Unit.name).all()
    selected_unit_id = request.args.get('unit_id', type=int)
    query = DailyManagement.query
    if selected_unit_id:
        query = query.filter(DailyManagement.unit_id == selected_unit_id)
    records = query.order_by(DailyManagement.manage_date.desc(), DailyManagement.id.desc()).limit(50).all()
    return render_template('management.html', units=units, records=records, today=date.today(), selected_unit_id=selected_unit_id)




ALLOWED_DOCUMENT_EXTENSIONS = {'.pdf', '.doc', '.docx', '.xls', '.xlsx', '.txt', '.png', '.jpg', '.jpeg'}
...
@login_required
@requires_permission('documents_manage')
def documents_page():
    if request.method == 'POST':
        ...
        if not allowed_document(uploaded.filename):
            flash('Formato não permitido. Use PDF, Word, Excel, TXT ou imagem.', 'danger')
            return redirect(url_for('documents_page'))
        ...
        if not content:
            ...
            return redirect(url_for('documents_page'))
        doc = FarmDocument(
            title=title or os.path.splitext(uploaded.filename)[0],
            category=category,
        ...
        db.session.add(doc)
        db.session.commit()
        flash('Documento da fazenda enviado com sucesso.', 'success')
        return redirect(url_for('documents_page'))

    category_filter = (request.args.get('category') or '').strip()
    search = (request.args.get('q') or '').strip()
    query = FarmDocument.query
    if category_filter:
        query = query.filter(FarmDocument.category == category_filter)
    if search:
        like = f'%{search}%'
        query = query.filter(db.or_(FarmDocument.title.ilike(like), FarmDocument.original_filename.ilike(like), FarmDocument.notes.ilike(like)))
    documents = query.order_by(FarmDocument.created_at.desc(), FarmDocument.id.desc()).all()
    categories = [c for (c,) in db.session.query(FarmDocument.category).distinct().order_by(FarmDocument.category).all() if c]
    return render_template('documents.html', documents=documents, categories=categories, selected_category=category_filter, search=search)
